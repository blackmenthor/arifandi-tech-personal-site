    ...
    if(this.props.data){
      ...
      var certification = this.props.data.certification.map(function(certification){
        return <div key={certification.title}><h3>{certification.title}</h3>
            <p className="info">{certification.publisher}<span>&bull;</span> <em className="date">{certification.validUntil}</em></p>
            <a href={certification.credentials} target='_blank'>See Credentials</a>
        </div>
      })
